...
int main(int argc, char *argv[])
{
  ...
  //Mark g_Y as write only and g_A and g_B as read only
  g_Y = clCreateBuffer(cv.context, CL_MEM_WRITE_ONLY, sizeof(float)*n,NULL,&err);
  CHK_ERR(err);
  g_A = clCreateBuffer(cv.context, CL_MEM_READ_ONLY, sizeof(float)*n,NULL,&err);
  CHK_ERR(err);
  g_B = clCreateBuffer(cv.context, CL_MEM_READ_ONLY, sizeof(float)*n,NULL,&err);
  CHK_ERR(err);
