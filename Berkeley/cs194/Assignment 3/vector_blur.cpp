  ...
  for(int r=0; r<n; r++)
    for(int c=0; c<n; c++){
      ...
      for(r2=iMin; r2<iMax; r2++) {
        for(c2 = jMin; c2+4<jMax; c2+=4){
	  sum0 = _mm_add_ps(sum0, _mm_loadu_ps(frame + r2*n + c2 + 0));

	  _mm_prefetch((__m128i*)(frame+ r2*n + c2 + 4), _MM_HINT_NTA);	  
        }	
        for(; c2<jMax; c2++){
	  sum0 = _mm_add_ss(sum0, _mm_load_ss(frame + r2*n + c2 + 0));
	...
	num += jMax - jMin;
      ...
      sum0 = _mm_hadd_ps(sum0, sum0);
      out[r*n+c] = _mm_cvtss_f32(sum0)/num;
    }  
}
