...
{
  for (int i = 1 ; i < SIZE-1 ; i++) {
    __builtin_prefetch(image1[i+1], 0, 0);
    __builtin_prefetch(image2[i+1], 1, 0);
    filter1(image1, image2, i);
  }
	
  for (int i = 2 ; i < SIZE-2 ; i ++) {
    __builtin_prefetch(image1[i+2], 0, 0);
    __builtin_prefetch(image2[i+2], 1, 0);
    filter2(image1, image2, i);
  }	
		
  for (int i = 1 ; i < SIZE-5 ; i ++) {
    __builtin_prefetch(image2[i+6], 0, 0);
    filter3(image2, i);
  }		
}
