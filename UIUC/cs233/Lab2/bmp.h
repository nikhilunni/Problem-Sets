...
inline pixel * bmp::operator()(int32_t x, int32_t y) {
	return const_cast<pixel *>((*const_cast<const bmp *>(this))(x, y));
}

inline int32_t bmp::width() const {
	return dib_header_.width;
}

inline int32_t bmp::height() const {
	return dib_header_.height;
}

inline void bmp::bmp_header::write_to_file(std::ofstream & file) {
	file.write(reinterpret_cast<char *>(this), sizeof(bmp_header));
}

inline void bmp::bitmap_info_header::write_to_file(std::ofstream & file) {
	file.write(reinterpret_cast<char *>(this), sizeof(bitmap_info_header));
}

inline uint32_t bmp::bitmap_info_header::compute_padding() {
