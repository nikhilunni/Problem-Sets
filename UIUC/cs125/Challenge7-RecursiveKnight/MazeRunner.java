//UIUC CS125 FALL 2013 MP. File: MazeRunner.java, CS125 Project: Challenge7-RecursiveKnight, Version: 2013-11-12T10:38:50-0600.575917765
/*
 * @author nunni2
 */
public class MazeRunner {
	...
	static int findShortestString(String[] paths, int lo, int hi) {
		if(lo>=hi)
			return lo;
		int smallestIndex = findShortestString(paths,lo+1,hi);
		return (safeStringLength(paths[lo]) < safeStringLength(paths[smallestIndex]) ? lo : smallestIndex);
	}
	...
	public static String shortestPath(int x, int y, int tX, int tY,
		...
		if(x < 0 || y < 0 || x >= blocked.length || y >= blocked[0].length
				|| blocked[x][y] == true || blocked[tX][tY] == true)
			return null;
		if(x == tX && y == tY)
			return "";
		blocked[x][y] = true;
		...
		paths[0] = (e != null ? "E"+e : null);
		paths[1] = (w != null ? "W"+w : null);
		paths[2] = (n != null ? "N"+n : null);
		paths[3] = (s != null ? "S"+s : null);

		blocked[x][y] = false;

		// TODO: Use findShortestString on paths
		// TODO: Return correct string with Compass direction prepended (or null)
		return paths[findShortestString(paths,0,paths.length-1)];
	}
	...
	public void chase(boolean maze[][], int targetX, int targetY) {
		String shortP = shortestPath(x,y,targetX,targetY,maze);
		if(shortP != null && shortP.length()>0)
			moveOne(shortP.charAt(0));
	}
