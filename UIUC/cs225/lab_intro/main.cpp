...
{
    PNG newImage(firstImage.width(),firstImage.height());
	...
	return newImage;
		...
		for(size_t y = 0; y < firstImage.height(); y++) {
			newImage(x,y)->red = (firstImage(x,y)->red + secondImage(x,y)->red) / 2;
			newImage(x,y)->green = (firstImage(x,y)->green + secondImage(x,y)->green) / 2;
			newImage(x,y)->blue = (firstImage(x,y)->blue + secondImage(x,y)->blue) / 2;	
		}
	}
    return firstImage;
}
