...
class BinaryTree
{
    private:
		...
		bool orderedHelper(Node * subRoot) const;
		T leftMost(Node * subRoot) const;
		T rightMost(Node * subRoot) const;
		void pathsHelper(Node * subRoot, list<T> l) const;
		int sumHelper(Node * subRoot, int depth) const;
