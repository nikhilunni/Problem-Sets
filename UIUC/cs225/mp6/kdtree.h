...
class KDTree
{
    public:
		...
		bool operator()(Point<Dim> first, Point<Dim> second);
    private:
		void swap(Point<Dim> & a, Point<Dim> & b);
		void sort(int lo, int right, int idx, int dim);
		void _construct(int lo, int hi, int dim);
		int partition(int lo, int right, int idx, int dim);
		void construct(int start, int end, int dim);
