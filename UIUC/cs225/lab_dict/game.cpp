...
{
	if(users.count(username) > 0) //If username exists
	{
		if(users.at(username).status == User::Status::ONLINE)
			return FAIL;
		else
		{	
...
int Game::logout_user(const string& username)
{
	...
	{
		if(users.at(username).status == User::Status::IN_GAME)
			return USER_IN_MATCH;
		else
		{
			users.at(username).status = User::Status::OFFLINE;
			return SUCCESS;
		}
	}
	return INVALID_USERNAME;
...
{
	if(parties.count(party_id) > 0)
	{
		bool everybodyFree = true;
		Party part = parties.at(party_id);
		for(std::vector<std::string>::iterator it = part.members.begin(); it != part.members.end(); ++it)
			if(users.count(*it) > 0 && users.at(*it).status == User::Status::IN_GAME)
				everybodyFree = false;
		if(everybodyFree)
		{
			Match newMatch;
			newMatch.add_party(parties.at(party_id));
			int randNum;
			matches[randNum] = newMatch;
			return randNum;	
		}
		else
			return 0;
...
{
	Party part;
	part.members = usernames;
	part.wins = 0;
	part.losses = 0;
	part.win_streak = 0;
	string randString;
	parties[randString] = part;
	return randString;
}
